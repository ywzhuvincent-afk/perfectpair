...
const typeCopy: Record<ContributionType, { title: string; description: string }> = {
  missing_product: { title: "Add a missing product", description: "Tell us about a bra, tights, leggings or jeans we do not yet list. A product link is enough; no receipt or photo is needed." },
  ...
  return <section className="contribution-workspace">
    ...
    <form className="contribution-form" onSubmit={submit}>
      <div className="fields three">
        <label>Category<select value={form.category} onChange={(event) => update("category", event.target.value as ContributionCategory)} disabled={form.type === "brand_claim"}><option value="bra">Bras</option><option value="tights">Tights</option><option value="leggings">Leggings</option><option value="jeans">Jeans</option>{form.type === "brand_claim" && <option value="all">All four categories</option>}</select></label>
