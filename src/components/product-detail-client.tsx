...
function ProductImage({ tights }: { tights: boolean }) {
  return <div className="detail-product-image"><Image src={tights ? "/images/perfectpair-tights-category.png" : "/images/perfectpair-bra-category.png"} alt="" fill sizes="208px" /></div>;
}
...
function TightsProductDetail({ product }: { product: TightsProduct }) {
  ...
  return <>
    ...
    <section className="product-hero"><ProductImage tights /><div><p className="eyebrow">Tights · {product.brand}{product.country ? ` · ${product.country}` : ""}</p><h1>{product.name}</h1><p className="product-lead">{product.denier} denier · {product.opacity.replace("_", " ")} · {product.sizeRange}</p><div className="detail-badges"><span>{hasCommunityScore ? `Community score ★ ${product.score.overall}` : "Community score pending"}</span><span>{product.score.reviewCount ? `${product.score.reviewCount} structured reviews` : "No moderated reviews yet"}</span><span>Version {product.productVersion}</span><span>Updated {product.updatedAt.slice(0, 10)}</span></div></div><aside className="detail-match"><p className="eyebrow">Your fit today</p><strong>{match.score}%</strong><span>Personal match</span><p>{match.suggestedSize ? `${match.suggestedSize} is a starting point, not a size guarantee.` : "Add a usual tights size or coverage preference for a more specific match."}</p></aside></section>
