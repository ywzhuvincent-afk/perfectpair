...
function SourceLayers({ product }: { product: CatalogProduct }) {
  ...
  const factSummary = product.features.length ? product.features.join(" · ") : "Only reviewed facts are retained; details not established by a source remain unlisted.";
  return <section className="fact-row source-layers"><article><p className="eyebrow">Manufacturer data</p><h3>What the maker publishes</h3><p>{factSummary}</p>{manufacturerSource?.sourceUrl ? <a className="source-link" href={manufacturerSource.sourceUrl} target="_blank" rel="noreferrer">Open official product source ↗</a> : <span>Source retained per field</span>}</article><article><p className="eyebrow">Community data</p><h3>What wearers report</h3><p>Comfort, fit, stay-put performance, durability and value are structured, moderated fields—not a generic star review.</p><span>Confidence: {product.score.confidence}</span></article><article><p className="eyebrow">Editorial & Lab data</p><h3>Reserved for method-led testing</h3><p>Independent testing appears only with a public method and stays separate from manufacturer and community claims.</p><span>Product version history retained</span></article><article><p className="eyebrow">Image rights</p><h3>{product.media?.length ? "Approved product imagery" : "Image not yet licensed"}</h3><p>{product.media?.length ? "This image is tied to a reviewed source and display right. It can be withdrawn if that right expires or is revoked." : "A real product image will appear here only after we record a display permission, an authorised feed, or an original-photo licence."}</p><span>Last record check: {product.updatedAt.slice(0, 10)}</span></article></section>;
}
