...
create table if not exists public.product_media_assets (
  ...
  tights_product_id uuid references public.tights_products(id) on delete cascade,
...
create unique index if not exists one_primary_tights_media_asset_idx
  on public.product_media_assets(tights_product_id)
...
create index if not exists product_media_assets_public_lookup_idx
  on public.product_media_assets(rights_status, licence_expires_at, product_id, tights_product_id);

alter table public.product_media_assets enable row level security;
create trigger product_media_assets_updated_at before update on public.product_media_assets
  for each row execute procedure public.set_updated_at();

comment on table public.product_media_assets is 'Approved product imagery only. Each asset has a product, source, display-right evidence, validity and revocation status. Never store user body photos or unlicensed image URLs.';
