...
type Candidate = { id: string; external_id: string; canonical_url: string; normalized_payload: Record<string, unknown> | null; quality_score: number | null; quality_reasons: string[]; blocking_issues: string[]; status: string; product_category: "bra" | "tights"; created_at: string; reviewer_note: string | null };
...
const emptyManualProduct: ManualProductForm = {
  ...
  braStyle: "", wire: "", cupConstruction: "", supportLevel: "", tightsStyle: "", denier: "", opacity: "", waist: "", toe: "",
};
...
export function AdminConsole() {
  ...
  async function prefillManualProduct() {
    ...
    try {
      ...
      const draft = body.data as { sourceUrl: string; brand?: string; name?: string; material?: string[]; sizeRange?: string; productIdentifier?: string; categoryHint?: "bra" | "tights"; denierHint?: number; warnings?: string[] };
      ...
      <details className="admin-source-form" open>
        ...
        <form onSubmit={addManualCandidate}>
          <div className="fields three">
            <label>类别 / Category<select value={manualProduct.category} onChange={(event) => updateManualProduct("category", event.target.value as ManualProductForm["category"])}><option value="bra">文胸 / Bra</option><option value="tights">丝袜 / Tights</option></select></label>
