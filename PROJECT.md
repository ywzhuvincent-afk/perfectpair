...
| 产品与品牌资料库 | 四品类架构已就绪 | 独立数据库 schema、来源证据、候选审核、人工发布；正式目录当前为 10 条（5 文胸 + 5 丝袜）。Leggings/Jeans 等待来源审核后发布。 |
...
2. 当前 10 款正式产品尚无获授权的真实商品图；系统已支持审核后的图片登记和展示，但图片许可/自有拍摄不能由代码代替。
3. 尚无长期运行的授权商业 Feed；Cron 基础设施已就绪，但不可把无授权来源接入自动抓取。
4. 真实用户评分、价格历史、库存状态及全品牌覆盖仍在建设；当前 10 条产品是高可信起点，不代表全量目录。
5. 跨设备账户同步、云端资料导出/删除 API 尚未上线，当前是本地优先资料体验。
